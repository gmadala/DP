...
describe('Controller: FloorCarCtrl', function () {
  ...
  var registerCommonTests = function() {
    ...
    describe('mileageExit function', function() {
      it('should exist', function() {
        expect(typeof scope.mileageExit).toBe('function');
      });

      it('shouldn\'t run if there is already a selected vehicle', function() {
        scope.data.$selectedVehicle = null;
        spyOn(blackbook, 'fetchVehicleTypeInfoForVin').andReturn();
        scope.mileageExit();
        expect(blackbook.fetchVehicleTypeInfoForVin).not.toHaveBeenCalled();
      });

      it('should run if mileage and selected vehicle are populated and form is valid', function() {
        scope.data.UnitMileage = 2345;
        scope.data.$selectedVehicle = {};
        scope.data.UnitVin = 'foo';
        spyOn(blackbook, 'fetchVehicleTypeInfoForVin').andReturn({ then: function() { angular.noop } });
        scope.mileageExit(mockForm);
        expect(blackbook.fetchVehicleTypeInfoForVin).toHaveBeenCalled();
      });

      describe('result handling', function() {
        beforeEach(function() {
          scope.data.UnitMileage = 3456;
          scope.data.$selectedVehicle = [   {
            "VinPos1To8": "JHMGE872",
            "VinYearCode": "9",
            "Make": "Honda",
          ...
          scope.data.UnitVin = 'JHMGE872';
        });

        it('should update the selected vehicle info and blackbook mileage if successful', function() {
          var result = scope.data.$selectedVehicle;

          spyOn(blackbook, 'fetchVehicleTypeInfoForVin').andReturn({
            then: function(s) {
              s(scope.data.$selectedVehicle);
            }
          });

          scope.mileageExit(mockForm);
          expect(blackbook.fetchVehicleTypeInfoForVin).toHaveBeenCalled();
          expect(scope.data.$selectedVehicle).toEqual(result);
          expect(scope.data.$blackbookMileage).toBe(3456);
        });

        it('should set blackbook mileage back to null if it fails', function() {
          spyOn(blackbook, 'fetchVehicleTypeInfoForVin').andReturn({
            then: function(s, f) {
              f(scope.data.$selectedVehicle);
            }
          });
