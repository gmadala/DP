...
describe('Service: banner', function () {
  ...
  describe('request function', function () {
    ...
    it('should make a GET call to the specified URL', function () {
      httpBackend.expectGET('/DSCConfigurationService/VirtualOfficeNotificationService.svc/msg').respond(successResponse);
    ...
    it('should give an empty message if malformed data is received', function () {
      httpBackend.expectGET('/DSCConfigurationService/VirtualOfficeNotificationService.svc/msg').respond({bad: 'data'});
      banner.fetch(function(text){
        expect(text).toEqual('');
      })
      expect(httpBackend.flush).not.toThrow();
    ...
    it('should give an empty message if error is returned', function () {
      httpBackend.expectGET('/DSCConfigurationService/VirtualOfficeNotificationService.svc/msg').respond(500);
      banner.fetch(function(text){
        expect(text).toEqual('');
      })
      expect(httpBackend.flush).not.toThrow();
